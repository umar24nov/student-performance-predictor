        ...
        {!result && !loading && (
          <>
            ...
            <div className="h-1 bg-white/6 rounded-full overflow-hidden mb-6">
              <div className="h-full bg-gradient-to-r from-violet-500 to-blue-500 rounded-full" style={{width:`${((step+1)/Q_STEPS.length)*100}%`}}/>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {cur.opts.map(o => (
                <button key={o.v} onClick={() => pick(o.v)}
                  className="py-3.5 px-4 rounded-xl border border-white/10 bg-white/3 text-sm font-semibold text-slate-300 hover:border-violet-500/40 hover:text-white hover:bg-violet-500/10">
                  {o.l}
                </button>
              ))}
            </div>
        ...
        {result && !loading && (
          <>
            ...
            <button onClick={onFullQuiz}
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-blue-500 to-violet-600 text-sm font-bold">
    ...
    <div className="relative z-10">
      ...
      <section className="relative overflow-hidden pt-16 sm:pt-24 pb-20 sm:pb-28">
        <div className="absolute inset-0 opacity-60 pointer-events-none"
          style={{background:"linear-gradient(-45deg, rgba(59,130,246,.12), rgba(139,92,246,.08), rgba(16,185,129,.06), rgba(59,130,246,.10))"}}/>
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-8">
              <button onClick={onStartQuiz}
                className="px-7 py-4 rounded-2xl bg-gradient-to-r from-blue-500 to-violet-600 font-bold text-base hover:brightness-110">
                🔮 Predict My Performance
              </button>
              <button onClick={onAuth}
                className="px-7 py-4 rounded-2xl border border-white/15 font-bold text-base text-slate-300 hover:border-white/30 hover:text-white">
                Log In / Sign Up
              </button>
            </div>
            <div className="flex items-center gap-6 flex-wrap text-xs text-slate-500">
              {[
                ...
                <div key={l} className="flex items-baseline gap-1">
                  <span className="text-lg font-extrabold text-white">{n}{s}</span>
                  <span>{l}</span>
                </div>
              ))}
            </div>
          ...
          <div className="hidden lg:block">
            <div className="relative">
              <div className="absolute -top-5 right-4 bg-[#0d1220] border border-emerald-500/30 rounded-xl px-4 py-2 text-sm font-semibold text-emerald-400 shadow-xl shadow-emerald-500/10 z-10">
                🎓 Pass · 88% confidence
              </div>
              ...
              <div className="absolute -bottom-4 left-4 bg-[#0d1220] border border-blue-500/30 rounded-xl px-4 py-2 text-sm font-semibold text-blue-400 shadow-xl">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
            ...
            {[
              ...
              <div key={n} className="relative bg-[#0d1220] border border-white/8 rounded-3xl p-7 text-center hover:border-white/15">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <button key={t} onClick={fn}
                className={`group relative bg-[#0d1220] border border-white/8 rounded-3xl p-6 text-left hover:border-white/15 overflow-hidden h-full`}>
                <div className={`absolute inset-0 bg-gradient-to-br ${TOOLKIT_STYLES[clr].grad} to-transparent`}/>
                <div className="relative">
                  ...
                  <span className={`text-xs font-bold ${TOOLKIT_STYLES[clr].txt} inline-block`}>Open →</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className={`bg-gradient-to-b ${r.grad} to-[#0d1220] border border-white/8 rounded-2xl p-6 hover:border-white/15`}>
      ...
      <section className="py-20 border-t border-white/6">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <div className="space-y-2.5">
            {faqs.map(([q,a],i) => (
              <div key={i} className={`bg-[#0d1220] border rounded-2xl overflow-hidden ${faqOpen===i ? "border-blue-500/30" : "border-white/8 hover:border-white/15"}`}>
                <button onClick={() => setFaqOpen(faqOpen===i ? null : i)}
                  className="w-full flex items-center justify-between p-5 text-left">
                  <span className="font-bold text-sm pr-4">{q}</span>
                  <span className={`text-slate-400 text-lg shrink-0 ${faqOpen===i?"rotate-45":""}`}>+</span>
                </button>
                {faqOpen===i && (
                  <p className="text-sm text-slate-400 leading-relaxed px-5 pb-5">{a}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="relative bg-gradient-to-br from-blue-500/10 via-violet-500/10 to-emerald-500/10 border border-blue-500/20 rounded-3xl p-12 sm:p-16 text-center overflow-hidden">
            ...
            <div className="relative">
              ...
              <button onClick={onStartQuiz}
                className="px-10 py-4 rounded-2xl bg-gradient-to-r from-blue-500 to-violet-600 font-bold text-lg hover:brightness-110">
                🔮 Start My Prediction
              </button>
